 ...
 */
#ifndef FUZZUF_CC_INCLUDE_UTILS_RANGE_TRAITS_HPP
#define FUZZUF_CC_INCLUDE_UTILS_RANGE_TRAITS_HPP
#include <cstddef>
#include <iterator>
#include <type_traits>
#include <utility>

#include "fuzzuf_cc/utils/check_capability.hpp"
#include "fuzzuf_cc/utils/type_traits/remove_cvr.hpp"
#include "fuzzuf_cc/utils/void_t.hpp"
namespace fuzzuf_cc::utils::range {
...
FUZZUF_CC_CHECK_CAPABILITY(
    HasInsertRange, has_insert_range,
    std::declval<T &>().insert(std::declval<T &>().begin(),
                               std::declval<T &>().begin(),
                               std::declval<T &>().begin()))
...
FUZZUF_CC_CHECK_CAPABILITY(
    HasAssign, has_assign,
    std::declval<T &>().assign(std::declval<T &>().begin(),
                               std::declval<T &>().begin()))
// Check if the type has member function clear()
FUZZUF_CC_CHECK_CAPABILITY(HasClear, has_clear, std::declval<T &>().clear())
// Check if the type has member function empty()
FUZZUF_CC_CHECK_CAPABILITY(HasEmpty, has_empty, std::declval<T &>().empty())
// Check if the type has member function size()
FUZZUF_CC_CHECK_CAPABILITY(HasSize, has_size, std::declval<T &>().size())
// Check if the type has member function data()
FUZZUF_CC_CHECK_CAPABILITY(HasData, has_data, std::declval<T &>().data())
// Check if the type has member function resize( size )
FUZZUF_CC_CHECK_CAPABILITY(HasResize, has_resize,
                           std::declval<T &>().resize(1u))

/**
 * Return the size of range
 * Roughly compatible to C++20 std::ranges::size
 * @tparam S Type with member function size()
 * @param r Value of S
 */
template <typename S>
auto rangeSize(const S &r) -> std::enable_if_t<has_size_v<S>,  // S has size()
                                               std::size_t> {
  return r.size();
}
 ...
 * Return the size of range
 * Roughly compatible to C++20 std::ranges::size
 * @tparam S Type with member function begin() and end()
 * @param r Value of S
 */
template <typename S>
auto rangeSize(const S &r)
    -> std::enable_if_t<!has_size_v<S>,  // R doesn't have size()
                        std::size_t> {
 ...
 * @tparam S Type with member function empty()
 * @param r Value of S
 */
template <typename S>
auto rangeEmpty(const S &r)
    -> std::enable_if_t<has_empty_v<S>,  // S has empty()
                        std::size_t> {
  return r.empty();
}
...
template <typename S>
auto rangeEmpty(const S &r)
    -> std::enable_if_t<!has_empty_v<S>,  // Sにはメンバ関数empty()がない
                        std::size_t> {
 ...
 * ( for the case S has clear() )
 * @tparam R A type that meets the standard range concept and has clear()
 * @param r Range to remove elements
 */
template <typename S>
auto clear(S &r) -> std::enable_if_t<has_clear_v<S>> {
  r.clear();
}

/**
 * remove all elements in the range
 * ( for the case S doesn't have clear() )
 ...
 * @tparam S Type that satisfies range concept
 * @tparam I Type that satisfies output iterator concept
 * @param r1 This value is assigned
 * @param r2 Assign to this value
 */
template <typename S, typename I>
auto copy(const S &r, I dest)
    -> std::enable_if_t<is_range_v<S> && !is_range_v<I> && is_iterator_v<I>> {
